...
import { getSession, updateSessionStatus } from '../session-service/service.js';
 ...
 */
export async function startMonitoring(session_id) {
    const session = await getSession(session_id);
    const addressEntry = await getAddressBySession(session_id);
    if (!addressEntry)
        throw new Error(`No deposit address registered for session: ${session_id}`);
 ...
 */
async function startEVMMonitoring(session_id, deposit_address, network, session) {
    const alchemyUrl = process.env[`ALCHEMY_WS_URL_${network}`];
    if (!alchemyUrl)
        throw new Error(`ALCHEMY_WS_URL_${network} not configured`);
    ...
    const poll = async () => {
        try {
            ...
            if (confirmations >= required_confirmations) {
                ...
                // In production: Import recordConfirmedDeposit properly
                // For now, assume it's part of the limit service (skipping for brevity of core refactor)
 ...
 */
async function startSolanaMonitoring(session_id, deposit_address, session) {
 ...
 */
export function stopMonitoring(session_id) {
