...
import { SessionStatus, ConnectionType, ComplianceMode } from '@prisma/client';
...
export async function createSession(params) {
    ...
    if (finalMode === ComplianceMode.VALIDATE && finalConnectionType === ConnectionType.EXCHANGE && !kyc_name) {
        const err = new Error('kyc_name is required when mode is validate and connection_type is exchange');
        err.status = 422;
    ...
    if (!orgConfig.supported_chains.includes(network)) {
        const err = new Error(`Network '${network}' is not enabled for this organisation`);
        err.status = 400;
    ...
    if (!orgConfig.supported_tokens.includes(token)) {
        const err = new Error(`Token '${token}' is not supported`);
        err.status = 400;
    ...
    const session_jwt = jwt.sign({
        ...
        network,
        token,
    }, privateKey, { algorithm: 'RS256', expiresIn: '18m' });
    // ── Issue Session Refresh Token (24 hours) ────────────────────────────────
    const refresh_token = jwt.sign({ session_id, organization_id, type: 'refresh' }, privateKey, { algorithm: 'RS256', expiresIn: '24h' });
...
export async function getSession(session_id) {
    ...
    if (!session) {
        const err = new Error(`Session not found: ${session_id}`);
        err.status = 404;
 ...
 */
export async function updateSessionStatus(session_id, new_status, updates = {}) {
...
export async function expireSession(session_id) {
    ...
    if (!session)
        return;
    const nonExpirable = [SessionStatus.COMPLETED, SessionStatus.FAILED, SessionStatus.EXTENDED_MONITORING];
    if (nonExpirable.includes(session.status))
        return;
    if (session.tx_hash) {
...
export async function refreshSession(refresh_token) {
    ...
    if (!privateKey)
        throw new Error('JWT_PRIVATE_KEY not configured');
    try {
        const payload = jwt.verify(refresh_token, privateKey, { algorithms: ['RS256'] });
