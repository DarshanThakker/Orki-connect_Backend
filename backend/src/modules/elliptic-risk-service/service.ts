 ...
 */
export async function screenWalletAddress(session_id: string, address: string) {
  if (MOCK_MODE) {
 ...
 */
function parseEllipticResponse(session_id: string, address: string, rawResponse: any) {
  ...
  const highRiskFlags = exposures
    ...
    .map((e: any) => ({ category: e.category, value: e.value, percentage: e.percentage }));
  ...
  const result = {
    ...
    flags: highRiskFlags.map((f: any) => f.category),
 ...
 */
function mockRiskScreen(session_id: string, address: string) {
