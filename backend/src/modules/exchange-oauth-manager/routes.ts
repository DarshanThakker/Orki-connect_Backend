...
router.get('/authorize', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const url = getAuthorizationUrl(exchange as string, session_id as string);
        res.redirect(url);
    } catch (err) {
        next(err);
    }
...
router.get('/callback', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = await handleOAuthCallback({
            ...
            authorization_code: authorization_code as string,
        });
 ...
 */
router.post('/withdraw', authMiddleware, async (req: any, res: Response, next: NextFunction) => {
    try {
