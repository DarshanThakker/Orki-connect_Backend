...
const VALIDATORS: Record<string, (addr: string) => boolean> = {
  [Chain.ETHEREUM]: (addr: string) => ethers.isAddress(addr),
  [Chain.POLYGON]: (addr: string) => ethers.isAddress(addr),
  [Chain.SOLANA]: (addr: string) => /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(addr), // Base58 validation
};

/**
 * Validates a deposit address for the given network.
 */
export function validateAddress(address: string, network: Chain) {
  const validator = VALIDATORS[network];
  if (!validator) {
    const err: any = new Error(`Unsupported network: ${network}`);
    err.status = 400;
  ...
  if (!validator(address)) {
    const err: any = new Error(`Invalid ${network} address format: ${address}`);
    err.status = 400;
