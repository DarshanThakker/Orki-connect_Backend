...
export async function enforceTransferLimits(params: { session_id: string; amount: string | number; exchange_fee?: number }) {
  ...
  if (numAmount < min_per_transaction) {
    throwLimitError(
      `Amount $${numAmount} is below the minimum transfer of $${min_per_transaction}`,
      'BELOW_MINIMUM',
      { amount: numAmount, minimum: min_per_transaction }
    );
  }

  // ── 2. Maximum per transaction ────────────────────────────────────────────
  if (numAmount > (max_per_transaction as number)) {
    throwLimitError(
      `Amount $${numAmount} exceeds the maximum transfer of $${max_per_transaction}`,
      'ABOVE_MAXIMUM',
      { amount: numAmount, maximum: max_per_transaction }
    );
  }
  ...
  if (exchange_fee > 0) {
    ...
    if (net_amount < min_per_transaction) {
      throwLimitError(
        `After exchange fee ($${exchange_fee}), net amount $${net_amount} falls below the minimum $${min_per_transaction}`,
        'NET_AMOUNT_BELOW_MINIMUM',
        { amount: numAmount, exchange_fee, net_amount, minimum: min_per_transaction }
      );
    }
  }
  ...
  if (currentDailyTotal + numAmount > (daily_user_limit as number)) {
    const remaining = (daily_user_limit as number) - currentDailyTotal;
    throwLimitError(
      `Transfer of $${numAmount} would exceed daily limit of $${daily_user_limit}. Remaining today: $${remaining}`,
      'DAILY_LIMIT_EXCEEDED',
      { amount: numAmount, daily_limit: daily_user_limit, current_total: currentDailyTotal, remaining }
    );
  }
  ...
  return {
    ...
    daily_total_after: currentDailyTotal + numAmount,
    remaining_daily: (daily_user_limit as number) - (currentDailyTotal + numAmount),
  };
}
 ...
 */
export async function getDailyUsage(organization_id: string, user_id: string) {
  const today = new Date().toISOString().slice(0, 10);
  const dailyKey = `${organization_id}:${user_id}:${today}`;
  return dailyTotals.get(dailyKey) || 0;
}

function throwLimitError(message: string, code: string, details: any = {}) {
